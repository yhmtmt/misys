...
void decodeAndPrintMessage1(FILE * ofile, char * str, int len)
{
	...
	fprintf(ofile,"%f,", turn);
	fprintf(ofile,"%f,", speed);
	fprintf(ofile,"%s,", accuracy ? "yes":"no");
	fprintf(ofile,"%f,", lon);
	fprintf(ofile,"%f,", lat);
	fprintf(ofile,"%f,", course);
	fprintf(ofile,"%u,", heading);
	fprintf(ofile,"%u,", second);
	fprintf(ofile,"%s,", (maneuver ? (maneuver == 1 ? "no special":"special"):"na"));
	fprintf(ofile,"%s,",raim ? "yes":"no");
	fprintf(ofile,"\n");
}
			...
			if(isAIVDM(str)){
				...
				if(field[5][0] == 1){ // message 1 has 1 in the first 6bit of field 6.
					for(i = 0; i < 8; i++){
						...
						if(i == 5){
							printf("Field[5]: ");
							decodeAndPrintMessage1(ofile, field[5], len);
							printf("\n");
						}else{
							printf("Field[%d]:%s\n", i, field[i]);
						}  
